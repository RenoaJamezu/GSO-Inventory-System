...
type AssetApi<TItem extends BaseAssetItem, TInput extends Record<string, unknown>> = {
  fetchAll: () => Promise<TItem[]>;
  create: (item: TInput) => Promise<TItem>;
  update: (id: number, item: TInput) => Promise<TItem>;
  remove: (id: number) => Promise<void>;
};

type AssetStore<TItem extends BaseAssetItem, TInput extends Record<string, unknown>> = {
  items: TItem[];
...
>(api: AssetApi<TItem, TInput>, entityLabel: string) {
  return create<AssetStore<TItem, TInput>>((set) => ({
    ...
    add: async (item) => {
      try {
        const nextItem = await api.create(item);
        ...
        const message =
          error instanceof Error
            ? error.message
            : `Failed to add ${entityLabel} record.`;
    ...
    update: async (id, item) => {
      try {
        const nextItem = await api.update(id, item);
        set((state) => ({
          items: state.items.map((row) => (row.id === id ? nextItem : row)),
        }));
        return { success: true, data: nextItem };
      } catch (error) {
        const message =
